...
1. **Reinstall the original app** from the Play Store (uninstall the patched one, install from Store).
2. Set proxy:  
...
If the emulator doesn’t allow running frida-server (no root), you may need to use **frida-gadget**: add the gadget to the **original** (unpatched) APK and load an SSL unpinning script when the app starts. See e.g. [Frida certificate pinning](https://httptoolkit.com/blog/frida-certificate-pinning/).
...
3. **Install the resulting APK** (uninstall current app first):  
